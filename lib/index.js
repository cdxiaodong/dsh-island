...
export class IslandService extends Service {
    ...
    constructor(ctx, config = {}) {
        ...
        ctx.on('internal/plugin', () => this.schedulePluginListRefresh());
        this.schedulePluginListRefresh();
        // 插件卸载时清理
        ctx.effect(() => () => {
            this.ctlServer?.close();
            this.ctlServer = undefined;
            if (this.pluginRefreshTimer)
                clearTimeout(this.pluginRefreshTimer);
        });
        // 会话生命周期
            ...
            conn.on('end', () => {
                try {
                    ...
                    switch (msg.type) {
                        ...
                        case 'plugin_disable':
                            if (msg.id)
                                this.disablePlugin(msg.id);
                            break;
                        case 'plugin_enable':
                            if (msg.id)
                                this.enablePlugin(msg.id);
                            break;
                    }
                }
                catch { /* ignore */ }
    ...
    disablePlugin(id) {
        ...
        if (!entry || id === 'dsh-island' || id === 'IslandService')
            return false;
        try {
            const registry = this.ctx.registry;
    ...
    enablePlugin(id) {
        try {
            ...
            if (saved) {
                ;
                this.ctx.plugin({ name: saved.name, apply: saved.callback, Config: saved.Config }, {});
            ...
            this.ctx.plugin({ name, apply: callback, Config }, {});
            this.debug('plugin_enable', id);
            return true;
        }
        catch {
